...
class VariationalNeuralBanditModel(BayesianNN):
  ...
  def build_model(self, activation_fn=tf.nn.relu):
    ...
    elbo = self.weighted_log_likelihood + (neg_kl_term / self.n)
  ...
  def train(self, data, num_steps):
    ...
    with self.graph.as_default():
      ...
      for step in range(num_steps):
        ...
        _, summary, global_step, loss, ll = self.sess.run(
            [self.train_op, self.summary_op, self.global_step, self.loss, self.weighted_log_likelihood],
            feed_dict={
                self.x: x,
                self.y: y,
